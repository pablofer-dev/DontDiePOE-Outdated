...
def elevate(path):
    ...
    ctypes.windll.shell32.ShellExecuteW(
        None, "runas", sys.executable, path, None, 1)
...
def press(vk, down=None):
    ...
    ctypes.windll.user32.SendInput(
        count, ctypes.byref(inputs), ctypes.sizeof(inputs))
...
def get_extended_tcp_table():
    ...
    ctypes.windll.iphlpapi.GetExtendedTcpTable(
        tcp_table, ctypes.byref(size), order, af, table_class, reserved)
    ...
    try:
        ...
        if ret:
            ctypes.windll.psapi.GetModuleBaseNameA(
                process, module, base_name, ctypes.sizeof(base_name))
